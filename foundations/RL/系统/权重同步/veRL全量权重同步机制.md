...
当前实现按 bucket 直接写入 live model，并没有构造完整 shadow model 后做一次原子指针交换。如果请求在安装过程中继续 forward，它理论上可能依次读到：
...
管理器通过暂停/abort 在途请求建立安全边界，使正常成功路径上的请求只观察到完整旧版本或完整新版本。

这是一种**控制面一致性**，不是底层参数存储的事务式原子更新。
