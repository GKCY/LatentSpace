...
description: 深入分析 T1 的任务合成、断言计数奖励、PPO 与 critic、TITO、MoE 路由重放和异步沙箱系统，核查性能收益、训练成本及复现边界。
date: 2026-09-14
tags:
  - RL
  - Agentic-RL
...
一个终端 Agent 花了半小时定位问题、修改代码、安装依赖，最终通过 10 项测试中的 7 项。如果训练只看任务是否完全成功，这次执行与一开始就失败的执行都可能得到零分。即便奖励能区分二者，训练器仍要确认：正在更新的 token、条件历史和 MoE 专家，是否对应模型当时真正执行过的那条轨迹。

[《T1: Terminal Agent Reinforcement Learning for Long-Horizon Tasks》](https://arxiv.org/html/2609.11042v1)把这些问题放进了一套完整的训练方案：用可执行任务提供部分完成信号，用全尺寸 critic 为每题一次采样提供 baseline，再用 TITO 和 rollout routing replay 约束训练数据与计算路径。其主要价值在于解释这些环节怎样相互依赖，以及系统跑起来后为什么仍可能学不到东西。

本文解读 Junyao Yang 等人的 **arXiv:2609.11042v1，2026 年 9 月 10 日版本**，并补充 RST、R³ 原始工作和作者公开产物。实验数值均来自作者报告，本文没有复现 GPU 训练。重点是这套 recipe 的机制、证据与可迁移条件。
...
| 环节          | 论文中的选择                                  | 对学习问题的影响                            |
| ------------- | --------------------------------------------- | ------------------------------------------- |
| 被训练的策略  | Qwen3.5-122B-A10B 的终端 Agent 策略           | 输入交互历史，输出文本和工具调用            |
| Agent harness | Harbor 承载的 Terminus-2                      | 管理工具执行、历史和上下文压缩              |
| 环境          | 每次 trial 一个 Daytona 云沙箱                | 文件、进程和命令结果构成持续变化的环境状态  |
| 训练任务      | 质量筛选后的 T1-15k                           | 每题含指令、环境、参考解和留出 verifier     |
| 优化算法      | PPO，全尺寸独立 critic                        | 每题每步采样一次，用价值函数提供 baseline   |
| 奖励          | 最终通过的断言数除以固定常数 20               | 未完整成功的轨迹也可能有正奖励              |
| 轨迹构造      | 保存真实 token、loss mask、logprob 和路由记录 | 工具观察提供上下文，生成 token 承担策略损失 |
| MoE 对齐      | TITO 加 R³                                    | 分别处理 token 漂移和专家选择差异           |
| 主运行规模    | 有效 batch 512，训练上下文上限 84k            | 采样启动 560 次，接纳先完成的 512 次        |
| 调度          | 作者报告为一步异步                            | 生成与训练使用独立设备池，按步发布权重      |

来源：[§2](https://arxiv.org/html/2609.11042v1#S2)、[§4](https://arxiv.org/html/2609.11042v1#S4)、[§6.2](https://arxiv.org/html/2609.11042v1#S6.SS2)、[§7.4](https://arxiv.org/html/2609.11042v1#S7.SS4)。84k 是训练序列预算；论文摘要中的 300+ 工具轮次、主评测配置和案例分析另有口径，后文分别讨论。

下面按一次任务的生命周期画出概念流程。一个 trial 是一道题的一次完整尝试；一个 turn 是其中一次模型交互；chunk 是轨迹无法继续拼接时导出的训练片段，三者不能混用。
...
T1-15k 按质量排名顺序存储。若顺序读取，同一 batch 会集中在很窄的质量区间，critic 看到的目标分布也会沿 epoch 变化。作者因此采用带种子的逐 epoch permutation，将不同质量和难度的任务打散。这项处理稳定的是启动任务的混合分布，后文的完成时间筛选还会改变最终进入训练的数据。[论文 §5.4](https://arxiv.org/html/2609.11042v1#S5.SS4)
...
T1-15k 也没有均匀覆盖各种终端任务。脚本与自动化、软件开发、系统管理、环境与包配置、版本控制五类合计占 67.9%。数据科学占 3.7%，调试占 1.3%，性能工作占 1.0%。这些比例能描述训练侧偏好，但单靠分类占比还无法解释某个能力增益的因果来源。[论文 §3.1、图 3](https://arxiv.org/html/2609.11042v1#S3.SS1)

作者声明训练任务与三个评测集隔离，种子及合成任务不来自 Terminal-Bench 2.1。它支持跨任务迁移的解释。更强的“完全分布外”表述仍需克制：训练与评测共享终端工程领域和 harness，任务不重叠并不意味着技能、环境结构或所有上游训练数据都不重叠。
...
### 3.2 “Dense”指分数细，不是每一轮发奖励

T1 在一次 trial 结束后执行 verifier，读取最终逐断言报告，再生成一个标量奖励。这个奖励放在每个 trajectory chunk 的最后一个 response token 上，其他位置不额外加入每轮奖励或 potential-based shaping。[论文 §2.2、§5.3](https://arxiv.org/html/2609.11042v1#S5.SS3)
...
## 4. 为什么愿意多养一个 122B critic

### 4.1 每题一次采样，把预算用于更多任务
...
### 4.2 GAE 在这个设定下接近“终局得分减去前缀预期”

记第 $j$ 个决策位置的状态为 $s_j$，奖励为 $r_j$，旧 critic 为 $V_{\mathrm{old}}$。T1 使用：
...
## 5. TITO：先保证训练的是采样出的动作

### 5.1 文本一样，token 不一定一样

令第 $i$ 轮实际输入为 $p_i$、采样输出为 $a_i$。严格 TITO 要求：
...
这里有一个必须保留的区别。论文在 1,402 个样本的审计中报告 **loss 区域 token 漂移率为 0%**；它保证参与动作损失的标识符来自真实采样。§10 同时承认，在 retokenized 情况下，训练用原始采样 token 作为后续历史，而 rollout 下一轮看过的是 harness 重分词后的历史，审计运行中有 2.6% 的 token 涉及这种情况。[论文 §4.2、§10](https://arxiv.org/html/2609.11042v1#S10)

因此可以把保证拆成两层：

$$
\text{动作 token 保真}\quad\text{与}\quad\text{条件前缀保真}.
$$

保住第一层非常有价值，第二层仍需要 harness 直接把 token 历史作为事实来源。本文据此把 T1 描述为“保留采样动作并修复拼接”，不将 0% 扩写成整个执行过程的分布完全一致。更一般的轨迹管理问题可结合 [[foundations/RL/系统/轨迹管理/什么是TITO|TITO 问题及解决方案]] 和 [[foundations/RL/recipe分析/slime-coding-agent-rl|Slime Coding-Agent RL Recipe 分析]] 阅读。

## 6. R³：对齐产生动作的专家路径

### 6.1 固定专家集合，继续训练 gate
...
### 6.3 0.021 降到 0.013，测量的是哪种差异

为避免训练步编号混淆，用 $b$ 表示生成这批数据的行为版本，$\theta$ 表示正在优化的参数。按附录 §12.2，T1 的 PPO ratio 为：
...
正文 §4.5、外层训练算法和附录 §12.4 描述的顺序一致：先缓存更新前的 $V_{\mathrm{old}}$，训练 critic，再让 actor 使用这份旧 value 构造优势。value clipping 也围绕同一份旧值展开。不能因为 critic 先更新，就把正在变化的新 value 混进本步的 GAE 和 clipping 基准。[论文 §4.5、附录 §12.4](https://arxiv.org/html/2609.11042v1#S12.SS4)
...
来源：[§4.4–4.5 与附录表 3](https://arxiv.org/html/2609.11042v1#S11)。引言中的“critic 是 actor 学习率的 30 倍”与实数不符；附录说明，critic 是 canonical learning rate $5\times10^{-7}$ 的 30 倍，actor 是其 2 倍。复现时应采用明确数值并核对发布配置。

### 7.2 EV 衡量预测能力，不直接衡量策略进步

论文用 explained variance（EV）观察 critic：
...
当回报方差非零时，EV 越高，表示减去 value 后剩余的方差越少；EV 为负，则说明当前 baseline 反而增加了残差方差。EV 没有有限下界，也不衡量恒定预测偏移，所以最好与 value loss、预测均值和回报尺度一起看。跨 DP、CP rank 统计时应先正确汇总方差，不能简单平均各 rank 的 EV。[论文 §4.5、式 19](https://arxiv.org/html/2609.11042v1#S4.SS5)
...
冷启动 critic 的 EV 最低达到 −33.6，58 个记录步中有 30 步为负；图中省略了部分极端负值阶段。warm-start 曲线没有落到负值，但它从接近零逐渐上升，中途还有回落，后期才主要处于较高区间。因此，正文“从第一步就稳定在 0.71–0.86”的概括强于图像实际支持的结论。
...
## 8. 长程训练的瓶颈如何改变 Recipe

### 8.1 激活 10B，不代表只要保存 10B 的训练状态
...
### 8.2 Recurrent block 不能直接照搬普通 CP
...
设备总数固定时，更多 CP 会挤占 DP。例如 batch 512、DP 2、micro-batch 为 1 时，每个副本需要串行累积 256 个 micro-batch。容量、并发和步耗时必须一起考虑；“更高并行度”本身并不保证更快。

### 8.3 最先完成的 512 条，不是随机抽到的 512 条
...
按表中数值，RL 阶段相对 SFT 的提升为：TB2.1 增加 14.6 个百分点、相对增加约 29.6%；LHTB 增加 4.3 个平均奖励点、相对增加约 18.2%；TBH 增加 9.7 个百分点、相对增加约 34.3%。这些相对变化均以对应 SFT 分数作分母自行计算。TB2.1 从 base 到最终的 20.2 个百分点中，RL 阶段占约 72.3%；这是该训练链路的阶段分解，无法据此分配一般意义上的 SFT 与 RL 投资回报。
...
另一次运行中，摘要请求长度与上下文预算不匹配，导致 98.3% 的完整摘要尝试失败；fallback 只保留很短的历史，Agent 反复做已经完成的工作，平均轮数从 22 增到 30。模型实际决策时已经丢失了关键记忆，事后把采样 token 完整送进训练器也补不回来。[论文 §9.2](https://arxiv.org/html/2609.11042v1#S9.SS2)

因此，当轮数上升时，要检查它发生在哪类状态：是否在解决更多要求，是否在绕着同一个错误重复执行，还是摘要把旧结论丢掉了。直接施加长度惩罚，可能会惩罚有用的尝试，也可能只是掩盖上下文管理故障。T1 的 122B 主配置没有开启长度 shaping。

### 10.2 大部分难度分组增益来自 Medium

§6.4 的分组案例中，Easy 组的三个 checkpoint 都是 100%；Medium 的 T1 / SFT / base 约为 78% / 58% / 56%；Hard 则约为 33% / 30% / 20%。RL 相对 SFT 的改善在 Medium 最大，最难组只提高约 3 个百分点。[论文 §6.4、图 15](https://arxiv.org/html/2609.11042v1#S6.SS4)
...
### 10.3 两个成功案例，分别说明策略调整与验收问题

附录的 `build-pov-ray` 案例中，旧下载地址无法取得目标版本。SFT 在最终报告里已经能指出问题，却继续重试失效路径；T1 改用可用镜像，65 turns 后通过三项断言。这个案例提示，知道应该换方法与在执行中及时换方法之间，存在可训练的行为差距。[论文附录 §13、图 17](https://arxiv.org/html/2609.11042v1#S13)
...
`polyglot-c-py` 更能检验论文的验收原则。两边都生成了能在 Python 和 C 中运行的程序，但 verifier 还要求目录只包含指定源文件。SFT 按公开示例编译后留下二进制，T1 将它删除，于是最终得分为 0 与 1。作者将后者解读为更重视交付环境。[论文附录 §13、图 18](https://arxiv.org/html/2609.11042v1#S13)
...
表中的补充实验是本文根据缺口提出的建议，不是论文已经完成的验证。verifier 的运行时完整性尤其重要：论文 §10 明确说，它在 Agent 可控制的沙箱中执行，没有运行时防篡改检测。当前防线主要来自任务筛选。测试数更多只提高评分分辨率，不自动提高测试可信度。[论文 §10](https://arxiv.org/html/2609.11042v1#S10)
...
这也改变了 recipe 的设计顺序。任务包应先能启动、能完成、能公平验收，再决定断言如何计分；轨迹应先能解释采样动作与训练位置的对应，再计算 PPO ratio；并发提高后，还要检查哪些任务被保留，而不只看每小时产出多少条轨迹。奖励、harness、采样调度和优化器共同定义了模型实际学习的问题。

若要借鉴 T1，我会优先复用它的三类明确约束：为任务要求设计可检查的部分完成信号；让 token 与 MoE 路由记录沿同一条数据管线流动；用足够多的不同任务训练经过校准的 critic。具体的 $S=20$、15 倍学习率、560 取 512、84k 上下文，则需要在新数据分布与设备条件下重新验证。

现有结果说明，在 SFT 起点上进行真实终端执行反馈训练，可以明显改善多个终端基准的表现。对于最困难任务、执行效率和严格预算匹配的模型排序，证据仍有限。把这些边界保留下来，才能看清这篇论文真正提供的东西：一套有代表性的长程 MoE PPO 实践，以及几条会直接影响下一次训练成败的失败经验。
