...
Slime 用新沙箱评测 patch，保存每轮实际生成的 token，再把分支轨迹交给 GRPO。训练侧存在一个分组问题：分支展开后，默认奖励归一化可能把不同题目的样本混在一起。第 5 节通过一个小例子说明这一问题的成因。
...
准备任务时，可先用空 patch 和已知正确 patch 各跑一次评分。如果两者都成功，测试可能没有覆盖问题；如果两者都失败，就需要先排查环境和评分脚本。
...
标准的同题 GRPO 先对 prompt $q$ 的 $G$ 次独立执行计算组内优势：
...
要保留同题 GRPO 的分组，可先按每道题的原始执行计算组内优势，再把同一次执行的优势分配给它导出的所有片段。验证用例应覆盖不同执行产生不同数量分支的情况。
...
因此，除了任务解决率，监控还应分别记录启动失败率、patch 应用率、评测解析失败率和超时率，以及每次执行的 Sample 数、有效 token 数与各阶段耗时。只看平均 reward，很难分清变化来自模型能力，还是数据分布与运行环境。
...
若正式实验采用同题 GRPO，应先修正分支展开后的奖励分组，并检查失败样本对 baseline 的影响。第 5 节的问题说明，轨迹能导出、loss 能计算之后，仍需沿着展平、归一化、mask 和归约的顺序检查，确认最终更新符合预期的 GRPO 定义。模型能提升多少，则需要训练和独立评测来验证。
