...
生成下一个 token 时，推理引擎会根据已有前缀 $s$，为每个候选 token $a$ 给出一个概率 $\mu(a\mid s)$。所有候选 token 的概率合在一起，就构成了一个**概率分布**。rollout 按这组概率抽取 token，因此称 $\mu$ 为**行为分布**：它决定了实际样本的来源。

前文的 $q(a\mid s)$ 则是训练引擎在同一份旧权重下给出的概率。这一步校正希望用已有的 rollout 样本，估计按 $q$ 计算的平均值，所以把 $q$ 叫作这里的**目标分布**。“目标”指这一步希望对齐的概率分布；后续当前策略 $p_\theta$ 相对 $q$ 的更新，由另一项比率 $r=p_\theta/q$ 处理。

假设每个 token 对应一个待求平均的量 $f(a)$。样本来自 $\mu$，但我们希望按 $q$ 求平均，就需要给每个样本的贡献乘上 $q/\mu$。前提是：凡是 $q$ 给出正概率的 token，$\mu$ 也必须有机会采到。这就是“目标分布被行为分布覆盖”的含义。满足这一条件时，有：
...
例如，在同一个前缀下，某个 token 的 rollout 概率为 $0.1$，旧训练策略给出的概率为 $0.2$。假想从这个前缀重复抽取 100 次下一个 token，按 $\mu$ 抽样，预期会见到它 10 次；按 $q$ 抽样，预期会见到它 20 次。因此，用 rollout 样本估计 $q$ 下的平均值时，要把这个 token 每次出现时的贡献乘以 $k=0.2/0.1=2$，补偿它较低的出现概率。反过来，如果两者分别是 $0.2$ 和 $0.1$，它在 rollout 中更容易出现，每次贡献就应乘以 $k=0.5$。
